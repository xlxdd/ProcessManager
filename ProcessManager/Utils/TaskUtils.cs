...
public static class TaskUtils
{
    ...
    /// <returns></returns>
    public static async Task<bool> FinishInTimeLimit(Action act,int timeLimit)
    {
    ...
    /// <returns></returns>
    public static async Task<bool> RetryManyTimes(Action act, int timeLimit,int times)
    {
        for(int i = 0; i < times; i++)
        {
            if(await FinishInTimeLimit(act, timeLimit))return true;
        }
        return false;
    }
}
